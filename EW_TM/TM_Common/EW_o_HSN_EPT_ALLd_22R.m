...
PLANE WAVES, REFLECTION and REFRACTION  (stage 13)
       Rolf Poddig
...
Last Changes on Jun. 01., 2019
%}

%{
                       History :

 01. Jun.  2019 : -  Code-Angleichung FERTIG ! 

 31. May   2019 : - BEACHTEN : Pfade für GUI-Mode ANPASSEN !!! S. EW_o_HSN_EPT_ALLd_22A
                        Rolf hat leider mit EW_o_HSN_EPT_ALLd_21A begonnen !  Erledigt,                 OK
                  - Code fertig bis S.u. !

 26. May   2019 : - Indentation fertig, besser angepasst als alt !    Code fertig bis S.u. !

 25. May   2019 : - weiter Code und Indentation (fertig bis line 2199) anpassen !

 24. May   2019 : - weiter Code und Indentation anpassen !

 17. May   2019 : - Beginn als Kopie von EW_o_HSN_EPT_ALLd_21A  : weitermachen als Rolfs Variante
                  - Code ANGLEICHEN AN EW_o_ESN_HPT_ALLd_24R, soweit möglich !   FERTIG BIS :
                    Zeilen-Nr in      EW_o_ESN_HPT_ALLd_24R          HIER = EW_o_HSN_EPT_ALLd_22R 
                                      389  436  471  519  627               240  287  323  369  475
                                      821  952 1235 1512 1726 2012          682  803 1003 1279 1486 1744
                                     2279 2462 2947                        2054 2222 2741 
                                   
 
                             ALT in EW_o_HSN_EPT_ALLd_21A :
 ...
 28. Okt.  2018 : - Angleichung an EW_o_ESN_HPT_ALLd_18R (Stand 28.Okt.2018)                            OK
	  ...
	  %elseif (strcmp(arg_list{1}, "Advanced"))
		...
		DEMO_or_TRUE_DATA = True;
		  if (strcmp(arg_list{2}, "fW"))
            ...
            % Rolf, 26. Mai 2019 :
            eps_r1_array = zeros(N_freq, 1, 'double');    mue_r1_array = zeros(N_freq, 1, 'double');
            kap_1_array = zeros(N_freq, 1, 'double');
            eps_r2_array = zeros(N_freq, 1, 'double');    mue_r2_array = zeros(N_freq, 1, 'double');
            kap_2_array = zeros(N_freq, 1, 'double');
            eps_r3_array = zeros(N_freq, 1, 'double');    mue_r3_array = zeros(N_freq, 1, 'double');
            kap_3_array = zeros(N_freq, 1, 'double');
            %
            % geht NICHT :
            %r12_parallel = zeros(N_freq, 1, 'double complex');    t12_parallel = zeros(N_freq, 1, 'double complex');
            %r23_parallel = zeros(N_freq, 1, 'double complex');    t23_parallel = zeros(N_freq, 1, 'double complex');
            % dann auch überflüssig :
            %r12_parallel = zeros(N_freq, 1, 'double');    t12_parallel = zeros(N_freq, 1, 'double');
            %r23_parallel = zeros(N_freq, 1, 'double');    t23_parallel = zeros(N_freq, 1, 'double');
            r12_parallel = zeros(N_freq, 1);    t12_parallel = zeros(N_freq, 1);
            r23_parallel = zeros(N_freq, 1);    t23_parallel = zeros(N_freq, 1);
            %
            %Für Medium 1
			  for i = 1:length(freq_array)
				for m = 1:length(medium_1)
					if ( (freq_array(i) > medium_1(m).f_min) && (freq_array(i) <= medium_1(m).f_max) )
						[eps_r1_array(i), mue_r1_array(i), kap_1_array(i)] = ...
                                               build_epsmuekap_head_GUI_01( freq_array(i), medium_1(m));
					endif
				endfor
			  endfor	
			%Für Medium 2
			  for i = 1:length(freq_array)
				for m = 1:length(medium_2)
					if ( (freq_array(i) > medium_2(m).f_min) && (freq_array(i) <= medium_2(m).f_max) )
						[eps_r2_array(i), mue_r2_array(i), kap_2_array(i)] = ...
                                               build_epsmuekap_head_GUI_01( freq_array(i), medium_2(m));
					endif
				endfor
			  endfor
			%Für Medium 3
			  for i = 1:length(freq_array)
				for m = 1:length(medium_3)
					if ( (freq_array(i) > medium_3(m).f_min) && (freq_array(i) <= medium_3(m).f_max) )
						[eps_r3_array(i), mue_r3_array(i), kap_3_array(i)] = ...
                                               build_epsmuekap_head_GUI_01( freq_array(i), medium_3(m));
					endif
				endfor
			  endfor
...
auto_scl_lam = True;		% so werden IMMER mindestens N Perioden räumlich in jedem Medium angezeigt
% Rolf, 11.Mai 2019 :
  if( freq_inc_flag)
     auto_scl_lam = False;
  endif
...
iruns_inc = 2;	%  alle N Perioden Winkel ändern  (bzw. die Frequenz !? Rolf)
%
% 14. Nov. 18 André: Asukommentiert wegen argv()
...
%    Rolf, WEG am 19.Mai 2019 : scl_zxg  nirgends definiert !!! ???
...
%graphics_toolkit( 'qt');         # unter Windows : Dann fast so gut wie fltk ;  
                                  # unter Linux : GLEICH SCHNELL wie fltk !  ABER KEINE PAUSE PER MENUE !
...
%  28.Okt.2018 für TM verschobene Positionen :
%annotation( figM, "textbox", [0.79  0.053  0.11  0.07],...% für TE
...
%
%              für HSN :
%    5 grosse 3D-arrays mit ALLEN Werten;
...
%
% für EPT :
%    10 grosse 3D-arrays mit ALLEN Werten;
%    nur im 1. Durchlauf BERECHNEN, bei Wiederholungen nur PLOTTEN
	...
	if(compute_Sheets)
	  ...
	  % Neu 06. Feb. 2019: Frequenz-Sweep        #######################################################
		...
		while(upd_BSe)
		  ...
		  %        Beginn Berechnungen für Border 2 
		  ...
		  %
		  %C_i2 = Cae23;	S_i2 = Sae23;       % NÖTIG ???  NEIN
		  %C_t2 = Cat23;    S_t2 = Sat23;       % NÖTIG ???  NEIN
		  %
		  ...
		  %
			if( Sae23 < .9999999 )	% kein 'streifender Einfall'
				...
				%
				F_23_paral = m_23 * ( k_I_t23 * C_beta_t23 - J * k_R_t23 * Cat23 ) / ...
									( k_I_e23 * C_beta_e23 - J * k_R_e23 * Cae23 );
			else
				% 'streifender Einfall' :    ACHTUNG ! CODE NOCH FALSCH ???????????????????????? :
      ...
      phi_0h_e2 = angle( exp( J * k_R_t12_sc * L_eff_2 ));      % für TE
      % Rolf, 01.Jun.2019 (vor die loop ;  für TM) :
      phi_0_e2H = phase_Amp_t_H + phi_0h_e2;
      ...
      %  innere for-loop über eine Periode, z.B. 100 Werte der Phase ############
        for ip = 1:i_phi
          %
          hco1 += dhco1;  
          %
          ...
          %
          % Neu 15. May 2017 für Border # 2   bei TM (01.Jun.2019 vor die loop) :
          %%phi_0_e2H = phase_Amp_t_H + angle( exp( J * k_R_t12_sc * L_eff_2 ));
          %
          phi_e2H = phi + phi_0_e2H;
          phi_e2 = phi_e2H - dphi_0_s_H;
          %
          ...
          %
            if(upd_ZHt)
              ...
              % Neu für Border 2 am 01. Jun 2019 (analog zu TE) :
              ...
              % hier ältere Bezeichnungen beibehalten :
              A1_r2_exp = exp( (kx_I_r23_sc * scl_om) .* XS + (ky_I_r23_sc * scl_om) .* YS_r2);
              A2_r2_cos = (k_R_r23_sc * scl_om) .* XS;
              A1_t2_exp = exp( (kx_I_t23_sc * scl_om) .* XS + (ky_I_t23_sc * scl_om) .* YS_t2);
              A2_t2_cos = (k_R_t23_sc * scl_om) .* XS;
              %
      ...
      % den/die Doppelpfeile aus der Phasen-for-loop herausnehmen (DAHINTER) :
          ...
          %
            if( !mod(N_periods, iruns_inc) )	%  alle 'iruns_inc Perioden' Winkel ändern
                if (first_run)
                  ...
                  r12_parallel(i_inc_var) = r12_paral;
                  t12_parallel(i_inc_var) = t12_paral;
                  ...
                  r23_parallel(i_inc_var) = r23_paral;
                  t23_parallel(i_inc_var) = t23_paral;
              ...
              %
                  ...
                  dfreq *= -1.;			 % Richtung Frequenz-Änderung umkehren
                    ...
                    if ( (first_run)  && (static_plots) )
                      [p_handles, p_errors] = additional_Plots_TM_1( legend_flag, uv_1, uv_2, ...
                           ...
                           r12_parallel, t12_parallel, r23_parallel, t23_parallel);
                      %
                      disp(' PAUSE !    Weiter mit Taste ...');
                      pause();
                      ...
                      %
                      first_run = False;        # SET permanently
                    endif       % (first_run)  && (static_plots) )
                  %
				...
				% weiter 2019-Jun-01
				  freq_eq = True	       % ??? FEHLT BEI TE !!! ???
                endif        % (alpha_inc_flag OR freq_inc_flag)
