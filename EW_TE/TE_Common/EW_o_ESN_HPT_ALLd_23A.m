...
%{
 ...
 04. Apr.  2019 :  - Umstellung auf neue Ordnerstruktur -> Pfade anpassen!
  ...
  if ( nargin() > 1)
    ...
    %
		if (strcmp( Laufvariable_str, "fF") )
            alpha_inc_flag = 1;
            alpha_e_min = 0.45 * pi;	alpha_e_max = 0.5 * pi;   alpha_inc = 0.5;   % (Inc. in Grad)
...
while( view_mode )
  ...
  if(compute_Sheets)
    ...
    while(freq_eq)
        ...
        disp(["Zeile 1706: om_ref: ", num2str(om_ref)]);
        ...
        %      Wellenzahl komplex :
        [k_R_e12  k_I_e12] = k_Wave_cmpl_scalar(om, eps_1, mue_1, kap_1);
        disp(["Zeile 1767: k_R_e12: nicht Frequenz-relativ:", num2str(k_R_e12)]);
        h100 = sprintf('%.15f', k_R_e12);
        save('-append', 'k_R_e12.txt', 'h100');
        %  ÄNDERN in 'Frequenz-relativ' :
        k_R_e12 /= scl_om;    k_I_e12 /= scl_om;
        h100 = sprintf('%.15f', scl_om);
        save('-append', 'scl_om.txt', 'h100');
        disp(["Zeile 1770: scl_om: Frequenz-relativ:", h100]);
        h100 = sprintf('%.15f', k_R_e12);
        save('-append', 'k_R_e12_scl.txt', 'h100');
        disp(["Zeile 1770: k_R_e12: Frequenz-relativ:", h100]);
        %
        %
        %k_10 = om * sqrt( mue_1 * eps_1 );	%    ALT   exakt
        k_10 = om_ref * sqrt( mue_1 * eps_1 );	% verlustloser Anteil ; bei om_ref
        k_10_q = k_10^2;
        disp(["k_10_q:         ", num2str(k_10_q)]);
        %p_10 = om * mue_1 * kap_1;	% Verlust-Anteil
        p_10_ref = om_ref * mue_1 * kap_1;	% Verlust-Anteil == negativer Im-Teil ; bei om_ref
        p_10 = p_10_ref / scl_om;      % 'Frequenz-relativ'
        p_15 = 0.5 * p_10;	% Hilfsvariable
        disp(["p_15:         ", num2str(p_15)]);
        %
        %h3 = k_10_q;    h4 = p_10;
        h1 = sprintf('%12.5e', k_10_q);
        h2 = sprintf('%12.5e', p_10);
...
disp(["Zeile 2151: k30:                             ", num2str(k_30)]);
disp(["Zeile 2154: k_R_e23:                         ", num2str(k_R_e23)]);
...
%
%disp(["Zeile 2193: k_I_t21: ", num2str(k_I_t21)]);
% auch benötigt :
        ...
        elseif(freq_inc_flag)
          ...
          if( !mod( N_periods, iruns_inc) )	%  alle 'iruns_inc Perioden' Winkel ändern
              ...
              if ( (freqWi_current >= freqWi_max) && (freqWi_inc > 0.) )
                  ...
                  disp("Line 3208");
                  if (freq_inc_flag)
					dfreq *= -1.;			 % Richtung Frequenz-Änderung umkehren -> ergibt Fehlermeldung: 
				  endif
                  disp("Line 3210");
                    # AB HIER ALLES NUR EINMAL und NUR WENN GEWÜNSCHT :
                    if ( (first_run == True)  && (static_plots == True) )
						disp(["k_R_t12_normal:       ", num2str(k_I_t12_normal)]);
