...
if (nargin() > 1)
			...
			for i = 1:length(freq_array)
				...
				for m = 1:length(medium_1)
					if ( freq_array(i) > medium_1(m).f_min && freq_array(i) <= medium_1(m).f_max )
						[eps_r1(i), mue_r1(i), kap_1(i)] = build_epsmuekap( freq_array(i), medium_1(m));
					endif
				endfor
			endfor
			...
			for i = 1:length(freq_array)
				...
				for m = 1:length(medium_2)
					if ( freq_array(i) > medium_2(m).f_min && freq_array(i) <= medium_2(m).f_max )
						[eps_r2(i), mue_r2(i), kap_2(i)] = build_epsmuekap( freq_array(i), medium_2(m));
					endif
				endfor
			endfor
			...
			for i = 1:length(freq_array)
				...
				for m = 1:length(medium_3)
					if ( freq_array(i) > medium_3(m).f_min && freq_array(i) <= medium_3(m).f_max )
						[eps_r3(i), mue_r3(i), kap_3(i)] = build_epsmuekap( freq_array(i), medium_3(m));
					endif
				endfor
			endfor
	...
	fflush (stdout);
	choice_mat = input("         Auswahl-Integer >  ");
...
h2 = [" Einfallswinkel = " sprintf('%9.3f', alpha_e12) " Grad "];
...
% Anfang neu 09. Dez.. 2017 :
    ...
    upd_ZHe = False;	upd_ZHr = False;	upd_ZHt = False;
      ...
      if(alpha_inc_flag)    
          if( !mod(N_periods, iruns_alpha_inc) )	%  alle 'iruns_alpha_inc Perioden' Winkel ändern
              if (alpha_first_run == True)
                  uv_1(i_alpha_e12) = alpha_e12 / radw;
                  k_R_t12_paral(i_alpha_e12) = k_R_t12 * scl_om;     % wieder die echte Bedeutung
                  k_I_t12_paral(i_alpha_e12) = k_I_t12 * scl_om;
                  alpha_t12_paral(i_alpha_e12) = alpha_t12;
                  psi_t12_paral(i_alpha_e12) = psi_t12;
                  r12_paral(i_alpha_e12) = r12_paral;
                  t12_paral(i_alpha_e12) = t12_paral;
                  Z_FW_t_tr(i_alpha_e12) = Z_W_2_tr;
                  Z_FW_t_lo(i_alpha_e12) = Z_W_2_lo;
                  %   Neu 20. Mai 2017 fuer Border # 2 :
                  uv_2(i_alpha_e12) = alpha_e23 / radw;
                  k_R_t23_paral(i_alpha_e12) = k_R_t23 * scl_om;     % wieder die echte Bedeutung
                  k_I_t23_paral(i_alpha_e12) = k_I_t23 * scl_om;
                  alpha_t23_paral(i_alpha_e12) = alpha_t23;
                  psi_t23_paral(i_alpha_e12) = psi_t23;
                  r23_paral(i_alpha_e12) = r23_paral;
                  t23_paral(i_alpha_e12) = t23_paral;
                  Z_FW_t2_tr(i_alpha_e12) = Z_W_3_tr;
                  Z_FW_t2_lo(i_alpha_e12) = Z_W_3_lo;
                  %    ende Neu
                  i_alpha_e12 += 1;
              endif
              if ( (alpha_e12 >= alpha_e_max) && (alpha_inc > 0.) )
                  alpha_inc *= -1.;
              ...
              elseif ( (alpha_e12 <= alpha_e_min) && (alpha_inc < 0.) )
                  alpha_inc *= -1.;
              endif
