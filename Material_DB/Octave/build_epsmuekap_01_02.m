...
%{
 ...
 07. Oct.  2018 :  - Abfrage von if() auf switch() geaendert
                   - Einfall : 'NEUE Funktions-Typen' : 'Duplikate' der Original-Typen ,
                                                         normiert  {  x == (f_akt - f_low) / (f_high - f_low)  } .
                     Diese Funktionen 
                                      sollen wie die Originale AUSSEHEN, wenn FREQUENZ LOGARITHMISCH GEMEINT ist :
                     ...
                     ??    33    linker Rand Abl=Par_2 bei x=0 linear bis x=Par_1, dann Parabel N-ter Ordnung bis x=1, Abl=0
...
% Ziel :   y( f_akt)  -->  y_2( log10( f_akt) )  o.ae. 
%u = f_akt - f_low;
%    if ( u < 1. )
%        u = 1.;
%    endif
...
eps_par_1 = emk_Data(4);      eps_par_2 = emk_Data(5);
  ...
  switch (eps_ftype)
    ...
    case 32          %  Typ 32  ???????    sieht fallend OK aus (07.Okt.18)
        f_h = eps_par_1;
        %x_h = eps_par_2;
        u_h = log( 1. + eps_par_2 * (e - 1.) );
            if ( u <= u_h )
                N_h = f_h * u_h / ( ( 1. - f_h) * ( 1. - u_h) );
                a_h = ( 1. - f_h) / (u_h)^(N_h);
                eps_r = 1. - a_h * ( u^(N_h) );
            else
                eps_r = f_h * ( 1. - u ) / (1. - u_h);
        ...
        %
        %
    case 33         %  Typ 33    ???????    fallend VOLLSCHROTT (07.Okt.18)
        f_h = eps_par_1;
        u_h = log( 1. + eps_par_2 * (e - 1.) );
            if ( u <= u_h )
                eps_r = 1. - u * ( 1. - f_h ) / u_h;
...
%            endif
        %
    elseif (kap_ftype == 5)         % FEHLT
        kappa = kappa_low;
    elseif (kap_ftype == 6)         % FEHLT
        kappa = kappa_low;
    else        %if (kap_ftype == 7)  % FEHLT / OK ??????
        x_05 = (x - 0.5)^2;
