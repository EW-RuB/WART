...
PLANE WAVES, REFLECTION and REFRACTION  (stage 14)
       Rolf Poddig

HERE :  Border 1 : TEM incident , Case TE ; delta {all media lossy}  ;
        E ortho ( sheets normal ) ;  H parall  ( sheets parallel )

Started on May  11., 2019
Last Changes on May  11., 2019
%}

%{
                       History :
                       
 05. Jun   2019:   - scl_om eingebaut in fakt_med_2, c_Amp_e20, phi_0h_e2, phih_COLORS_e, A1_e12_exp, A1_r12_exp, A2_e12_cos, A2_r12_cos: 
   
 11. May   2019 :  - Beginn als Kopie von EW_o_ESN_HPT_ALLd_23R.m ; hier weiter Rolf
                       Analyse : ZWEIMAL-Berechnen von scl_lam_123 wäre absurd
                                 Benutzung in k_I_e12_sc                          ?
                                 die Auskommentierung von 'D'  lines 1025 - 1138  ?
                      ==>  
                         1) scl_lam_123  NUR NOCH EINMAL berechnen, weiter HINTEN im Code                   OK
                         2) falls Frequenz läuft, KEIN scl_lam_123 möglich !!!    d.h. == 1.                ??
                                   ( erst einmal fest so gesetzt ! )
                      ==>
                         direkt hinter den Solver-Aufrufen und auch vorher suchen :
                         lauter Freq.-rel. Ausdrücke in den Feld-Berechnungen :
                         k_R_e12_sc ff, A1_e12_exp ff,  .......
                               ==> alle ... *scl_om  
                               ==> alle Wellen-Amplituden laufen korrekt mit der Frequenz !!!              OK
                                   UND alle Wellen-Phasen laufen korrekt mit der Frequenz !!!              OK
                    - F_23_senkr und F_21_senkr sind wohl FALSCHE ALTE FORMELN !!!!!!!!!!!!!!!!!!!!!!!!!


							ALT in EW_o_ESN_HPT_ALLd_23R.m :
							
 11. May   2019 :  - für Frequenz-Sweep BEGINNEN : Wellenlängen in den Sheets korrigieren !
                       scl_lam_123  WIRD ZWEIMAL BERECHNET ??? um Zeile 900 herum UND um Zeile 1900 ???
   
 02. May   2019 :  - für Frequenz-Sweep lauffähig machen : N_freq=200 und alpha_e_const=25 Grad eingebaut;
                     alle 'eps_r_array'-Abfragen abgefangen !  ==> DIALOG_Mode !
   
 01. May   2019 :  - Beginn als Kopie von EW_o_ESN_HPT_ALLd_23A.m ; hier weiter Rolf
                   - aktuelles Ziel : Pfad-Umleitungen NUR bei GUI-MODE !!!                                    OK
                       ==> ENDGÜLTIG DIALOG-MODE definieren !  NEBEN DEMO und ADVANCED (beide GUI)             OK
                   - nur kurz angesehen :
                       FORMELN Reflexions- und Transmissions-Faktoren an Grenzen 2/3 und 2/1-rückwärts
                       noch URALT / FALSCH !!!!!!!!!!!!!!!!


							ALT in EW_o_ESN_HPT_ALLd_23A/22A.m :
							
 04. Apr.  2019 :  - André : Umstellung auf neue Ordnerstruktur -> Pfade anpassen!
                       
 07. Feb.  2019 :  - Frequenz läuft !!!
 ...
 13. Nov.  2018 :  - Erweiterung um die argv()-Funktion fertig. 
                   ...
                   - Demo- und Advanced-Modi begonnen
...
% wechselt in das Verzeichnis des Octave-Skriptes
cd ../../EW_TE/TE_Common

% Octave-Files-Pfade
addpath("../../EW_Common/Octave/");
addpath("../../EW_Advanced/Octave/");
...
% Neu André 13.Nov.'18 :
  ...
  if ( nargin() > 1) 
    % NEU Rof 2019-05-01 : 
    %    1.)      
    GUI_MODE = True;    DIALOG_MODE = False;
    %    2.)      
    % wechselt in das Verzeichnis des Octave-Skriptes
    cd ../../EW_TE/TE_Common
    % Octave-File-Pfade
    ...
    %
	  ...
	  %elseif (strcmp(arg_list{1}, "Advanced"))
	    %disp('Advanced-Mode');
	    ...
	    DEMO_or_TRUE_DATA = True;
		  if (strcmp(arg_list{2}, "fW"))
			...
			freq = freq_array(1);
			freq_inc_flag = 1;
            freq_ref = sqrt((f_start^2 + f_end^2)/2);
			alpha_inc_flag = 0;
			  ...
			  for i = 1:length(freq_array)
				for m = 1:length(medium_1)
					if ( (freq_array(i) > medium_1(m).f_min) && (freq_array(i) <= medium_1(m).f_max) )
						[eps_r1_array(i), mue_r1_array(i), kap_1_array(i)] = build_epsmuekap_head_GUI_01( freq_array(i), medium_1(m));
					endif
				endfor
			  endfor
			%Für Medium 2
			  for i = 1:length(freq_array)
				for m = 1:length(medium_2)
					if ( (freq_array(i) > medium_2(m).f_min) && (freq_array(i) <= medium_2(m).f_max) )
						[eps_r2_array(i), mue_r2_array(i), kap_2_array(i)] = build_epsmuekap_head_GUI_01( freq_array(i), medium_2(m));
					endif
				endfor
			  endfor
			%Für Medium 3
			  for i = 1:length(freq_array)
				for m = 1:length(medium_3)
					if ( (freq_array(i) > medium_3(m).f_min) && (freq_array(i) <= medium_3(m).f_max) )
						[eps_r3_array(i), mue_r3_array(i), kap_3_array(i)] = build_epsmuekap_head_GUI_01( freq_array(i), medium_3(m));
					endif
				endfor
			  endfor
			...
			%Für Medium 1
			  for m = 1:length(medium_1)
				if ( (freq > medium_1(m).f_min) && (freq <= medium_1(m).f_max) )
					[eps_r1, mue_r1, kap_1] = build_epsmuekap_head_GUI_01( freq, medium_1(m));
				endif
			  endfor
			%Für Medium 2 
			  for m = 1:length(medium_2)
				if ( (freq > medium_2(m).f_min) && (freq <= medium_2(m).f_max) )
					[eps_r2, mue_r2, kap_2] = build_epsmuekap_head_GUI_01( freq, medium_2(m));
				endif
			  endfor
			%Für Medium 3
			  for m = 1:length(medium_3)
				if ( (freq > medium_3(m).f_min) && (freq <= medium_3(m).f_max) )
					[eps_r3, mue_r3, kap_3] = build_epsmuekap_head_GUI_01( freq, medium_3(m));
				endif
			  endfor
		  else
			freq = str2double( arg_list{6});
	...
	disp( "\t \t \t       Octave-GUI");
	disp( "\t \t { derzeit nur verfügbar OHNE Material-DB (wie im Demo-Mode) ! }");
    %
    % NEU Rof 2019-05-01 : 
    DIALOG_MODE = True;     GUI_MODE = False;
    %
	fflush (stdout);
    ...
    %freq = 2.99792458e5;   % = freq_low (300 kHz)
    %freq = 2.99792458e11;   % = freq_high (300 GHz)
    %freq = 2.99792458e14;   % = freq_ir (300 THz)
    ...
    %
		if (strcmp( Laufvariable_str, "fF") )
            alpha_inc_flag = 1;
            alpha_e_min = 0.;	
            %alpha_e_min = 35/180 *pi;
            alpha_e_max = 0.5 * pi;   alpha_inc = 0.5;   % (Inc. in Grad)
 		...
 		elseif (strcmp( Laufvariable_str, "fW") )
			...
			%
			%               Rolf, 2019-05-02 neu : 
			%  bei DEMO/Advanced bisher :
			%%N_freq = ((f_end - f_start)/freqWi_inc) + 1;
            % stattdessen :
			N_freq = 100;
            freqWi_inc = (f_end - f_start)/ (N_freq - 1);
            %
            alpha_e_const = 25.;
            %               weiter 2019-05-02
            %
			freq_array = linspace( f_start, f_end, N_freq+1);
...
Text_ESN_HPT_Bo1_1();
  ...
  if (! DEMO_or_TRUE_DATA)
	%disp("Here DEMO or DIALOG Mode");
	Help_Case_Texts_1( sub_case_str);
  endif



%              Ab hier aus der Frequenz abgeleitete Daten :
...
%11. Jul. 19 André: Hier auskommentiert; weiter unten eingefügt bzw. zusammengefasst!
%(Zeile 595)
% Rolf, 11.Mai 2019:
%{
  if( freq_inc_flag)
...
iruns_inc = 2;	%  alle N Perioden den Winkel ändern  (bzw. die Frequenz !? Rolf)
%
...
%
% 11. Mai 19 Rolf :  MUSS WIEDER HINEIN im DIALOG
  if (DIALOG_MODE)
    % 14. Nov. 18 André: Auskommentiert wegen argv() :  
...
% annot_11-13 für lambda 1-3:

annot_11 = annotation( figM, "textbox", [0.004  0.65  0.084  0.12]);
set( annot_11, 'fontsize', 12, 'fontweight', 'normal', 'interpreter', 'tex');
set( annot_11, 'horizontalalignment', 'center', 'verticalalignment', 'bottom');
%{
h2 = [ sprintf('%12.5e', 0.0) ' m '];
set( annot_11, 'string', [' \lambda_{1} = ' h2] );
%}

annot_12 = annotation( figM, "textbox", [0.004  0.43  0.084  0.12]);
set( annot_12, 'fontsize', 12, 'fontweight', 'normal', 'interpreter', 'tex');
set( annot_12, 'horizontalalignment', 'center', 'verticalalignment', 'bottom');
%{
h2 = [ sprintf('%12.5e', 0.0) ' m '];
set( annot_12, 'string', [' \lambda_{1} = ' h2] );
%}

annot_13 = annotation( figM, "textbox", [0.004  0.21  0.084  0.12]);
set( annot_13, 'fontsize', 12, 'fontweight', 'normal', 'interpreter', 'tex');
set( annot_13, 'horizontalalignment', 'center', 'verticalalignment', 'bottom');
%{
h2 = [ sprintf('%12.5e', 0.0) ' m '];
set( annot_13, 'string', [' \lambda_{1} = ' h2] );
%}
...
% weiter nach hinten, in Frequenz-abhgg. Teil :
%{
...
while( view_mode )
  ...
  if(compute_Sheets)
    ...
    while(freq_eq)
        ...
        %
        %  ÄNDERN in 'Frequenz-relativ' :              AB HIER ALLE k_R, k_I 'Frequenz-relativ' !
        ...
        lamb_e = 2. * pi / ( scl_om * k_R_e12 );    % korrekt mit RELATIVEN k_R
        ...
        h2 = [ sprintf('%12.5e', lamb_e12_SI) ' m '];
        set( annot_11, 'string', [' \lambda_{1} = ' h2] );
        ...
        %
        %      Wellenzahl komplex Welle Typ 1 :   auch 'Freq.-rel'
        ...
        %
        h2 = [ sprintf('%12.5e', lamb_t12_SI) ' m '];
        set( annot_12, 'string', [' \lambda_{2} = ' h2] );
        %
        ...
        %
        h2 = [ sprintf('%12.5e', lamb_t23_SI) ' m '];
        set( annot_13, 'string', [' \lambda_{3} = ' h2] );
        %
        ...
        %   (möglichst incl. Medium #3)
        %
        %   ALT :   Dies war ABSOLUT korrekt, ohne 'Frequenz-relativ' o. ä. :
        ...
        % 04. Jun. 2019 
        D_echt = ( layer2_width / scl_om ) * scl_lam_123 ;  % berechnete Koords. bei 0 ... 4 ; BEDEUTUNG SKALIERT !
        
        %{
        if (freq_inc_flag)
            %D_echt = layer2_width * lamb_t12_SI * scl_lam_2;
            %D_echt = layer2_width * scl_lam_2;
            D_echt = layer2_width * scl_lam_123;
            h30 = sprintf('%12.5e', lamb_t12_SI);
            disp(['lamb_t12_SI:               ', h30]);
        else
            D_echt = layer2_width * scl_lam_123;
        endif
        %}
        ...
        if (freq_inc_flag)
            ...
            D_h = (layer2_width / scl_om) * scl_lam_123;
        endif
		
        h200 = sprintf('%11.4e', D_h);
        disp(['D_h: ', h200]);
	...
	%        ...........
		...
		%   später eigentlich :
		%   ZE_e = Amp_e .* exp( k_I_e12_sc .* XS) .* cos(phi_e .- k_R_e12_sc .* XS);
        %  zur Beschleunigung bereits vorab :
		%A1_e12_exp = exp( k_I_e12_sc .* XS);	A2_e12_cos = k_R_e12_sc .* XS;
		%A1_r12_exp = exp( k_I_r12_sc .* XS);	A2_r12_cos = k_R_r12_sc .* XS;
        % Rolf 11. Mai 2019 :
        %{
		A1_e12_exp = exp( (k_I_e12_sc * scl_om) .* XS);   A2_e12_cos = (k_R_e12_sc * scl_om) .* XS;
		A1_r12_exp = exp( (k_I_r12_sc * scl_om) .* XS);   A2_r12_cos = (k_R_r12_sc * scl_om) .* XS;
        %}
        % André 05. Jun. 2019: wieder Frequenz-relativ 
        if (freq_inc_flag)
            A1_e12_exp = exp( (k_I_e12_sc * scl_om) .* XS);   A2_e12_cos = (k_R_e12_sc * scl_om) .* XS;
            A1_r12_exp = exp( (k_I_r12_sc * scl_om) .* XS);   A2_r12_cos = (k_R_r12_sc * scl_om) .* XS;
        else
...
%                  ####################################################################
    ...
    while(upd_BSe)
        ...
        %
        alpha_r23 = alpha_e23; % Reflexionsgesetz angenommen
        ...
        k_R_e21 = k_R_r23;          k_I_e21 = k_I_r23;      % alle 'Freq-rel'
        %
        ...
        %
        % auch benötigt :
        alpha_r21 = alpha_e21;      % wegen der RBs :
        k_R_r21 = k_R_e21;    k_I_r21 = k_I_e21;           %  alle 'Freq-rel'           
        ...
        lamb_t = 2. * pi / ( scl_om * k_R_t12);     % korrekt bei 'Freq-rel'
        ...
        % Rolf, 11. Mai 2019 :
        hc = Amp_e * exp( k_I_e12_sc * scl_om * s_len + J * phi_0_s );
        %
        ...
        % André 04. Jun. 2019: Versuch mit scl_om
        c_Amp_e20 = c_Amp_t12 * exp( ( k_I_t12_sc * C_psi_t12 - J * k_R_t12_sc ) * scl_om * L_eff_2 );
        %
...
% Ende neu 23. Aug. 2017
    ...
    % André, 04. Jun. 2019: Versuch mit scl_om
    phi_0h_e2 = angle( exp( J * k_R_t12_sc * scl_om * L_eff_2 ));
    

    %      Palette einfallender Wellen :
    %phih_COLORS_e = .6 * pi - k_R_e12_sc .* XS;   % Lage Rot besser auf Maxima  als mit 0.5
    % Rolf  11. Mai 2019 :
    %phih_COLORS_e = .6 * pi - (k_R_e12_sc * scl_om) .* XS;   % Lage Rot besser auf Maxima  als mit 0.5
    % André 05. Jun. 2019: Versuch mit scl_om
    %phih_COLORS_e = .6 * pi * scl_om - (k_R_e12_sc * scl_om) .* XS;   % Lage Rot besser auf Maxima  als mit 0.5
    % André 05. Jun. 2019: wieder Frequenz-relativ -> ohne scl_om
    if (freq_inc_flag)
        phih_COLORS_e = .6 * pi - (k_R_e12_sc * scl_om) .* XS;
    else
        phih_COLORS_e = .6 * pi - k_R_e12_sc .* XS;   % Lage Rot besser auf Maxima  als mit 0.5
    endif
    %
    ...
    % Rolf  11. Mai 2019 :
    %phih_COLORS_r = -0.25 * pi - (k_R_r12_sc * scl_om) .* XS;   % Rot recht gut auf Maxima
    % André 05. Jun. 2019: wieder Frequenz-relativ -> ohne scl_om
    if (freq_inc_flag)
        phih_COLORS_r = -0.25 * pi - (k_R_r12_sc * scl_om) .* XS;
    else
        phih_COLORS_r = -0.25 * pi - k_R_r12_sc .* XS;   % Rot recht gut auf Maxima
    endif
    %
    ...
    % Rolf  11. Mai 2019 :
    %phih_COLORS_r2 = -0.25 * pi - (k_R_r23_sc * scl_om) .* XS;   % Rot recht gut auf Maxima
    % André 05. Jun. 2019: wieder Frequenz-relativ -> ohne scl_om
    if (freq_inc_flag)
        phih_COLORS_r2 = -0.25 * pi - (k_R_r23_sc * scl_om) .* XS;
    else
        phih_COLORS_r2 = -0.25 * pi - k_R_r23_sc .* XS;   % Rot recht gut auf Maxima
    endif
    %
    ...
    % Rolf  11. Mai 2019 :
    %phih_COLORS_t = .425 * pi - (k_R_t12_sc * scl_om) .* XS;   % Violett sehr gut auf Maxima
    % André 05. Jun. 2019: wieder Frequenz-relativ -> ohne scl_om
    if (freq_inc_flag)
        phih_COLORS_t = .425 * pi - (k_R_t12_sc * scl_om) .* XS;
    else
        phih_COLORS_t = .425 * pi - k_R_t12_sc .* XS;   % Violett sehr gut auf Maxima
    endif
    %
    ...
    % Rolf  11. Mai 2019 :
    %phih_COLORS_t2 = .425 * pi - (k_R_t23_sc * scl_om) .* XS;   % Violett sehr gut auf Maxima
    % André 05. Jun. 2019: wieder Frequenz-relativ -> ohne scl_om
    if (freq_inc_flag)
        phih_COLORS_t2 = .425 * pi - (k_R_t23_sc * scl_om) .* XS;
    else
        phih_COLORS_t2 = .425 * pi - k_R_t23_sc .* XS;   % Violett sehr gut auf Maxima
    endif
    ...
    for ip = 1:i_phi
        ...
        %
            if(upd_ZHt)
                % ESN  'delta' :
                %
                %A1_t12_exp = exp( kx_I_t12_sc .* XS + ky_I_t12 .* YS_t);  % NICHT ky_I_t12_sc ???
                %A2_t12_cos = k_R_t12_sc .* XS;
                % Rolf  11. Mai 2019 :
                %A1_t12_exp = exp( (kx_I_t12_sc * scl_om) .* XS + (ky_I_t12_sc * scl_om) .* YS_t);
                %A2_t12_cos = (k_R_t12_sc * scl_om) .* XS;
                % 
                % André 05. Jun. 2019: wieder Frequenz-relativ:
                if (freq_inc_flag)
                    A1_t12_exp = exp( (kx_I_t12_sc * scl_om) .* XS + (ky_I_t12_sc * scl_om) .* YS_t);
                ...
                % Rolf  11. Mai 2019 :
                %A1_r23_exp = exp( (kx_I_r23_sc * scl_om) .* XS + (ky_I_r23_sc * scl_om) .* YS_r2);
                %A2_r23_cos = (k_R_r23_sc * scl_om) .* XS;
                %A1_t23_exp = exp( (kx_I_t23_sc * scl_om) .* XS + (ky_I_t23_sc * scl_om) .* YS_t2);
                %A2_t23_cos = (k_R_t23_sc * scl_om) .* XS;
                %
                % André 05. Jun. 2019: wieder Frequenz-relativ:
                if (freq_inc_flag)
                    A1_r23_exp = exp( (kx_I_r23_sc * scl_om) .* XS + (ky_I_r23_sc * scl_om) .* YS_r2);
      ...
      if(inc_flag)
        ...
        elseif(freq_inc_flag)
          ...
          if( !mod( N_periods, iruns_inc) )	%  alle 'iruns_inc Perioden' Winkel ändern
              ...
              if ( (cast(freqWi_current, "single") >= cast(freqWi_max, "single")) && (freqWi_inc > 0.) )
              ...
              elseif ( (freqWi_current <= freqWi_min) && (freqWi_inc < 0.) )
                  ...
                  if (freq_inc_flag)
                    dfreq *= -1.;
                  endif                  % Richtung Frequenz-Änderung umkehren
              endif
			...
			elseif (freq_inc_flag)
                ...
                h5 = [ sprintf('%12.5e', lamb_e12_SI) 'm'];
				set( annot_5, 'string', [ 
							  [' \epsilon_{ r,1} = ' h2 ]; ...
							  ' '; ...
							  [' \mu_{ r,1} = ' h3 ]; ...
                ...
                h5 = [ sprintf('%12.5e', lamb_t12_SI) 'm'];
				set( annot_6, 'string', [ 
							  [' \epsilon_{ r,2} = ' h2 ]; ...
							  ' '; ...
							  [' \mu_{ r,2} = ' h3 ]; ...
                ...
                h5 = [ sprintf('%12.5e', lamb_t23_SI) 'm'];
				set( annot_7, 'string', [ 
							  [' \epsilon_{ r,3} = ' h2 ]; ...
							  ' '; ...
							  [' \mu_{ r,3} = ' h3 ]; ...
